    ...
    this.populate = (function(self) {
                     ...
                     while(current_position < 360) {
                        stack.push(object_transform);
                            object_transform = mult(object_transform,rotate(current_position,1,0,0));
                            object_transform = mult(object_transform,translate(0,2,0));
...
function capped_cylinder () {
    ...
    this.populate = (function (self) {
                     ...
                     var object_transform = mat4();
                     self.m_tube.populate(self,10,object_transform);
                     object_transform = mult(object_transform,translate(0,0,0.5));
                     stack.push(object_transform);
...
function frustum (numRectangles, points_transform, upper_multiplier, lower_multiplier) {
    triangle_strip.call(this);
    if( !arguments.length) return;
    this.populate( this, numRectangles, points_transform, upper_multiplier, lower_multiplier );
    this.init_buffers();
}
inherit(frustum,triangle_strip);

frustum.prototype.populate = function( recipient, numRectangles, points_transform, upper_multiplier, lower_multiplier ) {
    var vertices = [];
    var strip_indices = [];
    var offset = recipient.vertices.length;		var index_offset = recipient.indices.length;
    var topIdx = 0; var bottomIdx = numRectangles;
    ...
    for( var i = 0; i < numRectangles; i++ )
    {
        vertices[topIdx] 	= vec3( upper_multiplier*Math.cos(2 * Math.PI * topIdx / numRectangles), upper_multiplier*Math.sin(2 * Math.PI * topIdx / numRectangles), .5 );
        recipient.texture_coords[topIdx + offset]    = vec2(0, topIdx / numRectangles );
        vertices[bottomIdx] = vec3( lower_multiplier*Math.cos(2 * Math.PI * topIdx / numRectangles), lower_multiplier*Math.sin(2 * Math.PI * topIdx / numRectangles), -.5 );
        recipient.texture_coords[bottomIdx + offset] = vec2(1, topIdx / numRectangles );
        strip_indices.push(topIdx++);
        strip_indices.push(bottomIdx++);
    }
...
function halfcube(points_transform)
{
    shape.call(this);
    this.populate( this, mat4() );
    this.init_buffers();
};
inherit(halfcube,shape)

halfcube.prototype.populate = function ( recipient, points_transform)
{
    // Vertices are duplicated many times to force flat shading
    recipient.vertices = [
                          // Front face
                          vec3(0, 0, 0),
                          vec3(1, 0, 0),
